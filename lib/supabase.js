...
export async function insertGame(gameData) {
  ...
  // Only store excitement_score if explicitly provided and we want to save it
  // For raw data storage, we omit this so it can be recalculated later
  if (gameData.excitement !== undefined && gameData.saveExcitementScore !== false) {
    gameRecord.excitement_score = gameData.excitement;
  }

  // Store raw probability data if provided
  if (gameData.probabilityData) {
    gameRecord.probability_data = gameData.probabilityData;
  }

  // Store game context data if provided
  if (gameData.gameContext) {
    gameRecord.game_context = gameData.gameContext;
  }
